...
- **Config**: `config.yaml` (see `config.example.yaml`) + `backend/config.py` — config is loaded into dataclasses (`Config`, `ServerConfig`, `AppConfig`, `StorageConfig`, `ModelConfig`, `PermissionsConfig`). Server host/port, default user, optional Obsidian vault path, `system_prompt_path` (loads `prompts/system.md` into `app.system_prompt`), storage paths, model provider name, permission defaults. Secrets via environment variables / `.env.example` (e.g. `${OPENAI_API_KEY}`). Dependencies: `requirements.txt` (aiohttp, pyyaml). Default storage backend in the example config is **file** (persistent under `data/`).
